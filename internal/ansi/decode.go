...
// returned Escape value is non-zero, then it represents a complete escape
...
//
// If the returned Escape value is 0, the caller MAY proceed to decode a UTF-8
// rune from p[n:]; if this rune turns out to be ESCape (U+001B), the caller MAY
// decide either to process it immediately, or whether to wait for additional
// input bytes which may complete an ESCape sequence.
